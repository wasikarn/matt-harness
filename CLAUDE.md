...
- **v0.35.7** — Plan-first reflex: pull sessions toward planning before implementation on the tasks that warrant it (owner-reported gap — work starting unplanned → re-fixing the same problems). Layer 1 doctrine rung in `METHODOLOGY.md` Rule 1 ("Plan mode is the implementation checkpoint" — one-way-door / wide-blast-radius on a code-editing task → enter plan mode; **suggest-strongly by default**, model auto-entry only for clear one-way doors or user-unsure; skip trivial/mechanical). Layer 2 repointed `flow-nudge` output to lead with plan-first (Shift+Tab / `EnterPlanMode` / `kbg:task-prep`), PRD pipeline demoted to the "feature to spec" branch; trigger regex unchanged; content-contract test added. Verified ceiling: no hook can set `permissionMode` (suggest-only), only the model's `EnterPlanMode` is "auto", `defaultMode:plan` is all-or-nothing. Reuses existing surfaces — no new hook/skill/gate. Honest: raises probability, not a guarantee; revisit → deferred stateful edit-time checkpoint if the nudge proves insufficient.
