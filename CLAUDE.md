...
Plugin manifest is the primary validation gate. `scripts/run-gauntlet.sh` runs plugin-validate + full shell-lint + JSON lint + harness-audit + the 23-file behavioral test suite (21 shell files via a for-loop, plus 2 Python files — memory-lint and compress-docs verify-preserved — via separate `if` blocks: deny-gate, advisory-sensor, session/stop-hook, harness-audit, review-pr convergence-state, review-pr bounded-auto-loop, inventory-witness canonical-mode, gauntlet test-wiring, and slash-command-script unit tests — see the script for the file list) in parallel. The old 204-test critical-hooks suite and eval dataset gate were deleted, not rebuilt: most of what they tested was L3/L4/L5 autonomy machinery retired by ADR 0006, and current coverage already exceeds them.
