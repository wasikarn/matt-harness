...
**Safety vs taste (do not conflate).** For ~25 surfaces this flag is reversible UX taste — flip it per the recorded reason. For **`recursive-improve` alone** it is a load-bearing instance of the autonomy invariant, guarded by audit **#32 (CRIT)** and recorded in **ADR 0002** (`cat "${KBG_PLUGIN_ROOT}/docs/adr/0002-autonomy-invariant.md"`). Never reason about `recursive-improve`'s flag through this taste criterion — #32 governs it.
