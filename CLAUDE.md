...
- **v0.68.73** — Inspected `nextjs-reviewer.md` directly (no fixtures — findings verified against primary sources or by reading the file, per `advisor()`'s steer that a fixture tests review behavior, not the factual/scoping bugs actually found). The 3 most load-bearing version-specific claims (fetch caching default flip in v15, Client Router Cache `staleTimes` change, `middleware.ts`→`proxy.ts` rename in v16) all checked out clean against real Next.js v16.2.9 docs. Fixed 4 smaller issues: a `route.tsx` typo (Route Handlers are `.ts`, never JSX) that contradicted the file's own correct worked example elsewhere; an unsupported "three backend targets" claim about `backend-patterns` (it names exactly two); and the real find — the agent's own step-1 scoping instruction and Scope table both omitted `proxy.ts` even though its Middleware section documents the v16 rename in detail, meaning on a v16 project the agent's own entry condition would silently skip the file its Middleware guidance exists to review (caught by `advisor()`, not the initial read). Also confirmed `nextjs-reviewer` has zero routing wiring into `review-pr`, unlike `python-reviewer`/`flutter-reviewer` — flagged but declined per Rule 2 (extension-based routing can't express a path-only trigger like this, and no evidence yet anyone's hit the gap).
- **v0.68.72** — Removed `flutter-reviewer` on user request, mid-consideration as the next `/review-fixtures` candidate. Flutter/Dart isn't an actual stack anywhere in this harness's real projects — `dart-flutter-patterns` was removed for the identical reason in v0.68.51. Deleted the agent file and swept all 10 live references (README, task-prep-checker, agent-authoring-conventions, kbg-help, inventory-boundary.sh, review-pr's routing table + reference, orchestrate's Bash-gated list + reference, agent-voice-extension, both manifests), fixed every dependent fleet-count (20→19 agents), regenerated BOUNDARY.md. Left CHANGELOG.md's and skill-improvement-batches.md's historical entries untouched, same convention as v0.68.51/v0.68.52.
- **v0.68.71** — Ran `/review-fixtures python-reviewer`, picked as a newly-wired `review-pr` route (per `skills/review-pr/SKILL.md`'s own note that python-reviewer/flutter-reviewer/typescript-reviewer were added to the by-extension routing table without ever being exercised — `nextjs-reviewer` isn't referenced in that file at all, corrected after initially conflating the two) with zero production history to mine. Per `advisor()`'s steer after two straight dispatch-heavy loops with zero agent-attributable findings, inverted the effort split: inspected the agent file directly, cross-checked against `code-reviewer.md`/`security-reviewer.md` as fleet precedent, dispatched only one confirmatory eval. Found and fixed 3 real bugs: "Use list comprehensions over C-style loops" was tagged HIGH (blocking), contradicting the agent's own Noise Control rule and `code-reviewer`'s convention of defaulting pure-style observations to MEDIUM — demoted. The canonical SQL-injection GOOD example used `%s` (psycopg2/MySQL paramstyle), verified via direct `sqlite3` execution to raise `OperationalError` against Python's own stdlib driver (`qmark` paramstyle) — added a driver-paramstyle caveat. The canonical bare-except GOOD example dereferenced `record.id`, verified via direct execution to raise a fresh unhandled `AttributeError` when `record` is a plain dict, masking the original exception — fixed by dropping the attribute access. A fourth hypothesis (CRITICAL/HIGH/MEDIUM vs `review-pr`'s Critical/Important/Minor vocabulary mismatch) was investigated and refuted: `code-reviewer`/`security-reviewer` use the identical internal vocabulary, and Phase 5 re-derives tiers by independent reasoning, not string-matching — not a live bug. The one confirmatory dispatch (a live sqlite3 fixture) validated the paramstyle fix and surfaced a real bonus bug outside scope (connection/cursor never closed, correctly avoiding the "`with sqlite3.connect()` closes the connection" trap).
- **v0.68.70** — Ran `/review-fixtures performance-optimizer`, picked as the last untested member of the fleet's 3 write-capable mutator agents, already carrying a confirmed unfixed 4th instance of the unconditional-`npx` bug — gated all 6 instances (including Lighthouse, closing an inconsistent carve-out). Also fixed a real correctness bug found by inspection: the Algorithmic Analysis table's "shallow copy or immer" for deep-clone replacement — verified live via Node that shallow copy shares nested references (mutating a nested field on the "copy" leaks into the original) — and the same zero-test-suite Success Metrics ambiguity already fixed once in refactor-cleaner. 3 prior loops showed fixtures don't discriminate on wrong-answer traps, so dropped baseline dispatches and targeted the agent's one binary self-stated guardrail instead: "can't measure the claimed improvement → report, don't apply." Eval-A (unmeasurable — a rate-limited third-party API with no sandbox key anywhere) held the guardrail exactly as designed (zero edits, confirmed via `git diff`) and went further than expected, independently finding that the current code's rate-limit compliance is already accidental, not enforced. Eval-B (measurable — real runnable `bench.js`) found and fixed a bug in the loop's own fixture: `bench.js` had the function under test inline-duplicated rather than imported, disconnecting the benchmark from the file being edited — a bug this loop's own pre-verification also carried and missed, caught only because the dispatched agent checked its own measurement tool before trusting it. Independently reproduced the fixed numbers (~163ms → ~2.9ms) myself afterward.
Older releases (v0.68.27–v0.68.69): a sustained `/review-fixtures` improve+optimize sweep across the fleet — build-error-resolver, refactor-cleaner, security-reviewer, backend-patterns/frontend-patterns builds and re-reviews, orchestrate, tech-humanize, task-prep, decide, ship-merge, address-review, silent-failure-hunter, requirement-analyst, typescript-reviewer, summarizer, blind-spot-hunter, code-implementer (×2), code-reviewer, and typescript-patterns, plus the `/review-fixtures` command's own workspace-schema generalization (v0.68.62/.63) and self-check (v0.68.61). Each loop found and fixed 1-5 real bugs, verified against fixtures or live tools (Postgres, Node, `tsc --strict`). Also: two Tathep framework-pattern removal batches (`adonisjs-patterns`/`dart-flutter-patterns`/`effect-ts-patterns`/`fastapi-patterns`/`hono-patterns`/`langchain-langgraph-patterns`/`tauri-v2-patterns` in v0.68.51, then `codebase-onboarding` in v0.68.52). See `CHANGELOG.md` for the full per-release record.
