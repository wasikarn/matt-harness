...
- **v0.68.25** — User reported Claude-created PRs/PR comments "barely ever match" the harness's own templates. Definition-level audit came back clean (no competing templates), so the gap is runtime/discoverability, not a template gap. Two confirmed fixes, no concrete inconsistent artifact available to pin down further: (1) `hotfix-reference.md`'s Phase 4 built its `gh pr create --body` as a raw ad-hoc string, bypassing `kbg:pr`'s Summary/Changes/Testing/Related-Issues structure with no documented-divergence note (unlike the file's own merge-command sync-seam comment) — fixed to reuse the same headings plus a hotfix-only Rollback section. (2) `flow-nudge.sh` nudges pure PR-creation asks toward `kbg:pr` but had no carve-out for "reply to the review"/"respond to reviewer feedback" — the model can't discover `/address-review` on its own (a command, not a listed skill) and can't self-invoke it (`disable-model-invocation: true`) either. Added a matching English+Thai carve-out, deliberately narrow (`review|reviewer|thread`, not `comment`/`feedback` — a first draft fired on "respond to the comment about the budget," caught by `advisor()`); 12 new test cases, 73/73 passing.
