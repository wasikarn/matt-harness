...
- **v0.35.5** — Fresh-context adversarial audit of v0.35.4's `task-prep` + `task-prep-checker` against the 8 Anthropic docs fetched this session. 6 fixes: C1 `task-prep-checker` added to orchestrate fleet catalogue (12→13-agent, was a real check-12 gap not the false positive v0.35.4 dismissed); C8 check 33 regex +`checker` (maker≠checker name-gate blind spot); C9 dropped redundant CLAUDE.md re-Read in Phase-0 (already in SessionStart context); C11 inlined the 9-field definitions into the skill + checker (bare path broke from foreign CWDs; the first `$CLAUDE_PLUGIN_ROOT`/Glob fix was doctrine-violating — caught by the fresh-context advocate before commit); C7 `model: opus` rationale documented; C2 threat-model note on the checker's prompt-defense baseline (prevents cargo-cult deletion). 4 no-fix-with-reason (C3/C4/C6/C10). C5 no-op + checker smokes verified post-restart; Phase E router smoke (5 redirect cases) verified post-v0.35.6-restart; auto-fire-block part moot (flag removed in v0.35.6).
- **v0.35.4** — Operationalized `docs/reference/task-handoff-template.md` as `skills/task-prep` (skill, `disable-model-invocation: true`, main-loop) + `agents/task-prep-checker` (read-only fresh-context verifier). Router-first: ~7/30 real-world cases (idea/multi-task/review/ship/no-repro/TDD/trivial) redirect before the 9-field flow. Task-type-aware `<done-when>` synthesis (test/perf/repro/behavior/colleague-test). Verified skills-vs-commands: commands not deprecated but merged into skills; skills recommended for new plugins; `disable-model-invocation` is a skill-only documented field.
- **v0.35.3** — Mined from a drill-down of three Anthropic docs. Propagated the reviewer over-reporting guardrail ("flag only correctness-affecting gaps, treat the rest as optional") to `typescript-reviewer`/`python-reviewer` (already in `code-reviewer`/`flutter-reviewer`); left `security-reviewer`/`silent-failure-hunter` high-recall. Recorded the conscious default-to-action vs no-model-self-start divergence in memory. Emphasis-register audit refuted delta #1 — kbg already calibrated on `IMPORTANT`/`CRITICAL`/`MUST`.
- **v0.35.2** — Added the `AskUserQuestion` escalation criterion (present-tense fork, no sensible default → ask; anticipatory/conditional → stay passive text) sourced from superpowers' `finishing-a-development-branch` skill. Fixed one vague footer branch (`address-review.md`'s "close the PR" → `` `gh pr close <n>` ``) flagged directly by the owner.
