...
- **v0.35.7–v0.35.9** — Plan-first reflex: pull sessions toward planning before implementation on the tasks that warrant it (owner-reported gap — work starting unplanned → re-fixing the same problems). Layer 1 doctrine rung in `METHODOLOGY.md` Rule 1 ("Plan mode is the implementation checkpoint" — one-way-door / wide-blast-radius on a code-editing task → enter plan mode; **suggest-strongly by default**, model auto-entry only for clear one-way doors or user-unsure; skip trivial/mechanical). Layer 2 repointed `flow-nudge` output to lead with plan-first (Shift+Tab / `EnterPlanMode` / `kbg:task-prep`), PRD pipeline demoted to the "feature to spec" branch. Verified ceiling: no hook can set `permissionMode` (suggest-only), only the model's `EnterPlanMode` is "auto", `defaultMode:plan` is all-or-nothing. **v0.35.8** widened the trigger (was tuned to kbg meta-work, silent on real-project phrasing — 8/8 confirmed; adversarial `advisor()` caught it, not the green suite). **v0.35.9** precision-fixed `build`→`build (a|an|the|out)` so CI-failure reports don't over-fire. Reuses existing surfaces — no new hook/skill/gate. Decision score plateaus ~92 (honest ceiling of suggest-only; a hard gate scores *lower*). Revisit → deferred **opt-in sentinel-gated** edit-time checkpoint if the nudge proves insufficient (Rule 2).
...
- **v0.34.x** — Advisory learn-nudge on `SessionEnd`; `review-pr`/`ship-merge` real-world fixes (per-PR review-state keying, verified-N/A CI/approval disposition); `to-prd` → `jira-acli` publishing handoff; dropped dead `tools:` frontmatter key (4 ECC skills); second-pass superpowers survey added `diagnosing-bugs/scripts/find-polluter.sh` (test-pollution bisection).
- **v0.31.x–v0.33.x** — `review-pr` language-reviewer routing (typescript/python/flutter) + per-finding adversarial verifier (v0.33.0); harness-wide content-accuracy audit (16 fixes across 6 surfaces) + performance-correctness distillations + hook shell-form→exec-form conversion (v0.32.x); hot-path/gauntlet/token-load perf refactor + stale "shellcheck long-pole" correction (v0.31.x).
