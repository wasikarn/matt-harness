...
- **v0.52.0-v0.52.2** — Hardened `inventory-boundary.sh` (the `BOUNDARY.md` generator): dropped a ~2,900-token duplicate list/table dump, fixed hook-description truncation, widened the hook glob to catch `.py` gates it was silently missing (`worktree-guard.py`), plus a same-day awk `END`-block double-print bug. Separately escalated the "route Jira/Confluence through jira-acli" doctrine from advisory to computational (`atlassian-mcp-gate.sh`, a PreToolUse cold-start block) — the advisory-only version had already proven insufficient in practice.
