...
- **v0.51.0** — Added `requirement-analyst` agent (fleet +1): senior-level requirement analysis of a Jira ticket/Confluence spec/PRD/pasted text handed to it as text — ambiguities, missing ACs, edge cases, dependencies, testability, readiness verdict. Read-only (`Read`/`Grep`/`Glob`, no `Bash`) — it never fetches Jira/Confluence itself (a subagent has no `Skill` tool, so self-fetch would bypass the `jira-acli:acli` routing rule); filing findings back to a ticket stays `jira-acli:jira-content`'s job. Same-session follow-up wired it in opt-in/Jira-detection-gated: `code-reviewer` gained a **requirement-coverage lens** (fleet lenses 4→5, diff-vs-requirements, must check beyond the diff before flagging a gap — pre-existing code counts); `review-pr` gained **Phase 1.5** (fetch + dispatch + feed Phase 3/4's coverage routing, ticket-quality report shown separately from code findings in Phase 6); `task-prep` gained **Step 3.5** (feeds Step 5 auto-fill + Step 6 gap-asking). `advisor()` caught a real bug pre-ship: the ticket-quality section would've been auto-posted into the GitHub review body (this repo is public) — fixed to terminal-only, never posted, on both review targets.
