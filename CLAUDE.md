...
- **v0.45.0** — Migrated `prototype` and `resolving-merge-conflicts` out of kbg's vendored `skills/` tree (fleet 52→50) to `gh skill install mattpocock/skills engineering/<name> --agent claude-code --scope user`: both were self-contained, zero-kbg-value-add forks, and `gh skill` gives per-skill install + native drift detection (`gh skill update`) that the plugin system can't. `research`/`code-review`/`wayfinder` stay vendored — each carries real kbg-specific cross-references or specializations that a bare install would lose. Rewired the 4 `kbg:prototype` cross-refs in `ask-matt`/`wayfinder` to the bare `prototype` name.
