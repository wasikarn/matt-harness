...
regardless of weighted sum. If `qmd` is not configured or is unavailable, mark the criterion
`ข้อมูลไม่เพียงพอ` per the rule above rather than scoring from memory. (Adapted from
semantica-agi/semantica's `find_precedents` lifecycle step.)
...
**Named models** (cc-thinking-skills): the weighted-criteria + ranking loop is *tournament* (N options scored on the same rubric, winner by weighted sum) + *steel-manning* (the strongest version of each option is what gets scored — don't straw-man); the "single criterion that flips the verdict" trace is *leverage-points* (find the small number of places a change has outsized effect). Catalog + honesty caveat: read via Bash with `cat "${MH_PLUGIN_ROOT}/docs/reference/reasoning-models.md"`.
