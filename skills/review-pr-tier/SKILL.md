...
2. **Apply SCRUTINIZE-4 to every finding** (4 falsifiable checks, not vibes). A finding that fails any check goes to `.scratch/review-pr-<timestamp>/rejected.md` with the failing question + reason; the user sees the rejection tally, not the dropped finding:
   ...
   | 4 | **Evidence requirement** — what supports the claim? | Finding has `file:line` (or commit SHA) + the *minimal* command/output that confirms it | Claim is plausible but unverified — judge on the evidence itself, not the number: a coverage-mode finding tagged `Reviewer-Confidence: 40-79%` (`code-reviewer`'s per-finding tag, not its skill/agent-file frontmatter — see `Skill(kbg:review-lens-code-quality)`) still needs real `file:line` + command/output to survive Q4; a standalone `code-reviewer` dispatch never emits below 80% at all, so this question rarely fires there |
...
3.6. **Zero-findings adversarial re-hunt — the blind-spot guard** (why it exists + the agent's own traits: `../review-pr/reference.md` § Phase 5 step 3.6 — standalone form). When **zero
   ...
   *assume a defect exists and go find it*. Returns structured findings (possibly none).
   ...
   - **Returns nothing** → the zero-findings clean pass stands, now backed by an independent adversarial pass; record that the re-hunt ran (`kbg:review-pr-finish`'s Phase 6 surfaces it). Set `REHUNT_STATUS=clean`.
   - **Skip on a trivial diff** (a single non-test file — Rule 2). **Scoped to step 3.6's own dispatch only** — not a license to skip/substitute agents elsewhere (Phase 3's fan-out in `kbg:review-pr` has its own, narrower trivial-diff rule). Set `REHUNT_STATUS=skipped-trivial`.
   - **Skip because a Critical/Important finding already survives step 3.5** — no zero-findings signal exists to guard. Set `REHUNT_STATUS=n/a`.
   - Hunter errors, times out, or returns unparseable output → **re-hunt-not-run**, not a clean result (Phase 4 step 4's principle). Surface it so the clean verdict isn't overstated. Set `REHUNT_STATUS=incomplete`.
   - **`REHUNT_STATUS` must be one of these four exact literals** (`clean|skipped-trivial|incomplete|n/a`, per `write-review-state.sh`'s own usage header) — any other string (e.g. free-text like "re-hunt-not-run") silently falls through to `incomplete` in the script, which forces `CLEAN=false` downstream even where the semantics above call for `clean` or `n/a`.
