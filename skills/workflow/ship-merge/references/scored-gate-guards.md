...
- **(a) keyword regex** `auth|secret|credential|payment|billing|token`, case-insensitive —
  `skills/risk-check/SKILL.md` and `hooks/gates/verifier-protect.sh` fold case for the same
...
- **(b) the harness's own verifier/gate paths**, classified by running each path through
  ...
  — the same classifier `hooks/gates/verifier-protect.sh` and `skills/risk-check/SKILL.md`
