...
- Deadline: <N> min wall-clock — past it, the orchestrator stops this agent and re-dispatches with a narrower brief; a read-only dispatch may instead get a narrower duplicate sent while the original still runs (liveness is read from owned files' mtimes or `git status --porcelain -- <owned paths>`, never from this agent's transcript)
```
