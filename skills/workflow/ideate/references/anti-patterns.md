...
- **Collapsing Phase 1 + Phase 3 into one wave.** The 2-wave
  ...
  SKILL.md's "2-wave fan-out (load-bearing)" section for the full
...
- **Silent parse failures.** If a Diverge branch returns empty
  ...
  `mh:silent-failure-hunter` and SKILL.md's "3-axis scoring rubric"
...
- **Same-model judge treated as ground truth.** Phase 2 + 3 scoring runs on
  ...
  truth. Full reasoning: SKILL.md's "Phase 2 — Focus" section.
