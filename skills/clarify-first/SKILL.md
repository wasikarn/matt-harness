...
|---|---|---|
| Architecture / design trade-off | **Probe** (`kbg:decide` probe mode) | Why? → What if? → Shared fate → Contradiction |
...
**Rule**: `kbg:decide` probe mode is read-only analysis — use it when the user asks "should we..." or "what if...". Use `clarify-first` when Claude needs the user to fill a gap.
