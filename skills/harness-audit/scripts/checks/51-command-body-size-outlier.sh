#!/usr/bin/env bash
# 51. Command body size outlier — flag command entrypoints large enough to
# warrant a token-optimizer pass or a directory-form split. INFO only, never
# a hard cap: mirrors check 42's threshold for the identical reason (some
# commands are legitimately dense). Same 20K-char threshold as check 42 —
...
# Gap this closes: check 42 only globs skills/*/SKILL.md, so a command
