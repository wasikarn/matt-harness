# 54. fact-force-gate structured-decision contract (ADR 0007) — the four-fact
...
# friction that protects nothing (the exact third-party-port class, ADR 0007
...
if [ -f "$_F" ]; then
  ...
  printf '%s\n' "$_code" | /usr/bin/grep -qE 'hook_decision deny' \
    || crit "fact-force-gate.sh: deny path does not call _lib's hook_decision — a hand-rolled top-level permissionDecision is silently ignored (ADR 0007 contract #1)"
  printf '%s\n' "$_code" | /usr/bin/grep -qE 'KBG_GATEGUARD=off|KBG_FACT_FORCE_DISABLED' \
    || warn "fact-force-gate.sh: no off-switch (KBG_GATEGUARD=off / KBG_FACT_FORCE_DISABLED) — operator cannot disarm (ADR 0007)"
fi