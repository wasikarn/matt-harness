...
if [ -f "$ADR0004" ]; then
  # Gate-2 RETIRED 2026-06-25 (ADR 0004); 48a/48b push-gate runtime probes removed — 48c/48d retained (autonomy_on + L4 machinery NOT retired per ADR 0004).
  ...
  while IFS= read -r _f; do
    ...
    case "$_f" in
      ...
      *) case "$_base" in _lib.sh|loop-guard.py) _newok=1 ;; *) _newok=0 ;; esac ;;
    esac
