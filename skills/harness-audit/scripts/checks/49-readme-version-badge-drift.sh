#!/usr/bin/env bash
# 49. README version-badge drift. The README's shields.io version badge is a
...
if [ "$_is_kbg" = "1" ] && command -v jq >/dev/null 2>&1; then
  ...
  if [ -n "$_manifest_version" ] && [ -f "$_readme" ]; then
    ...
    if [ -z "$_badge_line" ]; then
      warn "readme version-badge check 49: no shields.io version badge found in README.md — location may have moved/been reworded"
    else
      case "$_badge_line" in
