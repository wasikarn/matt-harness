#!/usr/bin/env bash
# 55. Fleet model:/effort: frontmatter — every surface entrypoint carries both keys.
#
# Convention: docs/skill-authoring-conventions.md "Explicit model: + effort: on
# every surface (fleet convention, v0.68.430)" — all agents/*.md, commands/*.md
# + commands/*/COMMAND.md, and skills/*/SKILL.md ship with explicit model: and
...
# commands/*/references/*.md fragments stay unstamped by design (check 42 owns
# the reference-file frontmatter class).
#
# WARN (not CRIT): convention drift, not the tamper-sensitive class 39/44/49 guard.
shopt -s nullglob
_me_files=("$CLAUDE_DIR"/agents/*.md "$CLAUDE_DIR"/commands/*.md "$CLAUDE_DIR"/commands/*/COMMAND.md "$CLAUDE_DIR"/skills/*/SKILL.md)
shopt -u nullglob
