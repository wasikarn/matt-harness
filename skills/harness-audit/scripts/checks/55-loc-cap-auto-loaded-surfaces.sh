#!/usr/bin/env bash
# 55. LOC cap on auto-loaded surface files — agents/*.md,
# skills/*/SKILL.md must stay <= 200 lines (plain
...
# Coexists with checks 38/51 (20K-char INFO) — chars catch dense long
...
# not lines). (commands/*.md and commands/*/COMMAND.md dropped from this
...
for _f in "$CLAUDE_DIR"/agents/*.md \
  ...
  if [ "$_lines" -gt "$LOC_CAP" ]; then
    _chars=$(wc -c < "$_f" | tr -d ' ')
    _rel=${_f#"$CLAUDE_DIR"/}
    warn "'$_rel' is ${_lines} lines (cap ${LOC_CAP}; ${_chars} chars) — move detail to the surface's reference file (skills: reference.md; agents: compress, cuts listed in the commit message)"
  fi
done
