# 53. Profile-ladder floor coverage (ADR 0007) — `CLAUDE_HOOK_PROFILE=minimal`
...
# ADR 0007 exists to protect. CRIT, not WARN: the floor is the load-bearing
...
if [ -f "$_LIB" ]; then
  sed '/^[[:space:]]*#/d' "$_LIB" 2>/dev/null | /usr/bin/grep -qE 'HOOK_PROFILES:-standard strict' \
    || crit "profile ladder: _lib.sh hook_init has no HOOK_PROFILES tier (default 'standard strict') — profile gating is inert (ADR 0007)"
fi
for _g in "${_FLOOR_GATES[@]}"; do
  _f=$(find "$CLAUDE_DIR/hooks" -type f -name "$_g.sh" 2>/dev/null | head -1)
  [ -f "$_f" ] || { crit "profile ladder: floor gate '$_g.sh' missing (ADR 0007 floor)"; continue; }
  sed '/^[[:space:]]*#/d' "$_f" 2>/dev/null | /usr/bin/grep -qE 'HOOK_PROFILES="minimal standard strict"' \
    || crit "profile ladder: floor gate '$_g.sh' missing HOOK_PROFILES=\"minimal standard strict\" — a minimal session loses this floor gate (ADR 0007)"
done