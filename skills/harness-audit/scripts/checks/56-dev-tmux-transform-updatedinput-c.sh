# 56. dev-tmux-transform updatedInput contract (CLAUDE.md §Hook architecture (current profile ladder design)) — rewrites a dev-server
...
# shipped (CLAUDE.md §Hook architecture (current profile ladder design) contract #1); the bash port was built to the correct field.
...
if [ -f "$_F" ]; then
  sed '/^[[:space:]]*#/d' "$_F" 2>/dev/null | /usr/bin/grep -qE 'hookSpecificOutput.*updatedInput' \
    || crit "dev-tmux-transform.sh: rewrite does not use hookSpecificOutput.updatedInput — a top-level command mutation is silently ignored (CLAUDE.md §Hook architecture (current profile ladder design) contract #1); the agent blocks on a foreground dev server"
fi