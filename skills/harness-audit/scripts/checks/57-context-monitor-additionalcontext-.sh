# 57. context-monitor additionalContext contract + advisory-only (CLAUDE.md §Hook architecture (current profile ladder design)) —
...
if [ -f "$_F" ]; then
  /usr/bin/grep -qE 'hookSpecificOutput.*additionalContext' "$_F" 2>/dev/null \
    || crit "context-monitor.sh: advisory does not use hookSpecificOutput.additionalContext — a top-level additionalContext is silently ignored (CLAUDE.md §Hook architecture (current profile ladder design) contract #2)"
  /usr/bin/grep -qE 'hookEventName.*PostToolUse' "$_F" 2>/dev/null \
    || crit "context-monitor.sh: additionalContext missing hookEventName:\"PostToolUse\" — advisory is dropped (CLAUDE.md §Hook architecture (current profile ladder design) contract #2)"
  ...
  if /usr/bin/grep -qE 'permissionDecision' "$_F" 2>/dev/null; then
    crit "context-monitor.sh: emits permissionDecision — a PostToolUse advisory must be observe-only (CLAUDE.md §Hook architecture (current profile ladder design)); a deny here is a category error"
  fi
fi