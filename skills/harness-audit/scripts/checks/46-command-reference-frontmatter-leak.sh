#!/usr/bin/env bash
# 46. Command reference-file frontmatter leak.
#
# A directory-form command (`commands/<name>/COMMAND.md` + a `references/`
# subfolder, e.g. `ship`/`ideate`) is meant to keep its supporting files
# inert — read only via an explicit path pointer in the parent COMMAND.md's
# prose, never invoked on their own. Confirmed via a live
# `claude -p "..." --debug-file <path>` capture (2026-07-20 commands/
# deep-research pass): `commands/ship/references/classify.md` and
...
shopt -s nullglob
_leak_candidates=("$CLAUDE_DIR"/commands/*/*.md "$CLAUDE_DIR"/commands/*/*/*.md)
shopt -u nullglob
for _f in "${_leak_candidates[@]}"; do
  [ -f "$_f" ] || continue
  case "$(basename "$_f")" in COMMAND.md) continue ;; esac
  grep -q '^---' "$_f" || continue
  if [ -n "$(fm_get "$_f" "description" --block)" ]; then
    warn "reference file ${_f#"$CLAUDE_DIR"/} carries frontmatter with a description: — Claude Code loads it as an independent command, not inert supporting material (strip the frontmatter, matching commands/ideate/references/frames.md's shape, or move the file outside commands/)"
  fi
done
