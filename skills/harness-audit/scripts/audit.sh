...
# ── checks (sourced in filename-glob order; fragments are numbered 1..61 with
# filename prefix == '# N.' header, so glob-sorting by 01..61 sources them in
...
# Split-integrity guard: exactly 61 fragments, each carrying one '# N.' header,
# numbers 1..61 each exactly once. Source scope == count scope == the SAME glob
