...
# ── checks (sourced in filename-glob order; fragments are numbered 1..59 with
# filename prefix == '# N.' header, so glob-sorting by 01..59 sources them in
...
# Split-integrity guard: exactly 59 fragments, each carrying one '# N.' header,
# numbers 1..59 each exactly once. Source scope == count scope == the SAME glob
