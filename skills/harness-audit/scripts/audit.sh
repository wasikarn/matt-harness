...
# ── checks (sourced in filename-glob order; fragments are numbered 1..53 with
# filename prefix == '# N.' header, so glob-sorting by 01..53 sources them in
...
# Split-integrity guard: exactly 53 fragments, each carrying one '# N.' header,
# numbers 1..53 each exactly once. Source scope == count scope == the SAME glob
