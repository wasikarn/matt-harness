...
# ── checks (sourced in filename-glob order; fragments are numbered 1..54 with
# filename prefix == '# N.' header, so glob-sorting by 01..54 sources them in
...
# Split-integrity guard: exactly 54 fragments, each carrying one '# N.' header,
# numbers 1..54 each exactly once. Source scope == count scope == the SAME glob
