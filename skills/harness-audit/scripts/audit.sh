...
# ── checks (sourced in filename-glob order; fragments are numbered 1..58 with
# filename prefix == '# N.' header, so glob-sorting by 01..58 sources them in
...
# Split-integrity guard: exactly 58 fragments, each carrying one '# N.' header,
# numbers 1..58 each exactly once. Source scope == count scope == the SAME glob
