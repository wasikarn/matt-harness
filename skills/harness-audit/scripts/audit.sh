...
# ── checks (sourced in filename-glob order; fragments are numbered 1..60 with
# filename prefix == '# N.' header, so glob-sorting by 01..60 sources them in
...
# Split-integrity guard: exactly 60 fragments, each carrying one '# N.' header,
# numbers 1..60 each exactly once. Source scope == count scope == the SAME glob
