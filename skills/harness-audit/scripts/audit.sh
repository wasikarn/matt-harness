...
# ── checks (sourced in filename-glob order; fragments are numbered 1..62 with
# filename prefix == '# N.' header, so glob-sorting by 01..62 sources them in
...
# Split-integrity guard: exactly 62 fragments, each carrying one '# N.' header,
# numbers 1..62 each exactly once. Source scope == count scope == the SAME glob
