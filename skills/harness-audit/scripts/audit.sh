...
# ── checks (sourced in filename-glob order; fragments are numbered 1..34 with
# filename prefix == '# N.' header, so glob-sorting by 01..34 sources them in
...
# Split-integrity guard: exactly 34 fragments, each carrying one '# N.' header,
# numbers 1..34 each exactly once. Source scope == count scope == the SAME glob
