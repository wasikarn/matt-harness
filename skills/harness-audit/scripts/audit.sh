...
# ── checks (sourced in filename-glob order; fragments are numbered 1..35 with
# filename prefix == '# N.' header, so glob-sorting by 01..35 sources them in
...
# Split-integrity guard: exactly 36 fragments, each carrying one '# N.' header,
# numbers 1..36 each exactly once. Source scope == count scope == the SAME glob
