...
**If Step 3.5 ran**, its `open_questions` are load-bearing gap candidates too — a ticket ambiguity a downstream implementer would hit is exactly the class of gap this step exists to surface before send, not after. Fold them into this batch alongside the four above. **If Step 3.5 flagged a `riskiest_assumption`**, treat it as its own priority tier ahead of the generic `open_questions` list — it's the analyst's single designated top risk, not one more item that has to win a tie-break against everything else on the list; don't let a task-threatening assumption get demoted to a passing `<edge-cases>` mention just because ten lower-stakes `open_questions` are competing for the same ask-slot. **If that pushes the total past 4 ask-worthy fields** (the four above, plus Step 3.5's `riskiest_assumption`/`open_questions` and `<edge-cases>` below), the ≤4 batch cap forces a cut. Extend the priority order past `<artifacts>`: Step 3.5's `riskiest_assumption` next (if flagged), then generic `open_questions` (a concrete, ticket-sourced ambiguity outranks generic foresight-gathering), then `<edge-cases>` last. Ask the top 4, note in the emitted prompt that N more remain open, and pick them up in the re-verify pass (Step 10) — don't silently drop them.
