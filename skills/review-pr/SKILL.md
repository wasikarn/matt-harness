...
1. Present findings in this format:
   ...
   **Acceptance-contract check** (only if the task locked one via `/accept-task`): look for `.scratch/<slug>/ACCEPTANCE.md`. If absent, skip this silently. If present, cross-check each Critical/Important finding against the locked `## Criteria`:
   ...
   Surface a one-line count above the decision: `Acceptance: 4/5 criteria met · 1 gap (must-fix)`. This is the verification half of the contract `/accept-task` wrote — it catches "looks done but missed the brief" before merge, not after.
