...
description: "Diagnostics-only BGP troubleshooting patterns for neighbor state, route exchange, prefix policy, AS path inspection, and safe evidence collection. Use when a BGP session is down/flapping, prefixes are not propagating, AS-path policy is suspected, neighbor state needs interpretation, or the user says 'BGP down', 'route not propagating', 'prefix rejected', 'neighbor flapping'. Thai: 'BGP down', 'BGP neighbor', 'แก้ BGP', 'prefix ไม่มา'. Don't use for: live config changes (read-only diagnostics), enterprise network architecture (use network-architect), or general routing questions unrelated to BGP (use networking-engineer). Evidence-first: collect show output before proposing root cause."
---
