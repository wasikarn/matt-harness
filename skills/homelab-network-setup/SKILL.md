...
description: "Practical home and homelab network planning for gateways, switches, access points, IP ranges, DHCP reservations, DNS, cabling, and common beginner mistakes. Use when the user is designing a home/small-lab network, choosing between UniFi/OPNsense/pfSense, planning Pi-hole + NAS topology, picking IP ranges that won't conflict with corporate VPN, or troubleshooting home wifi coverage. Thai: 'homelab', 'เครือข่ายบ้าน', 'วาง network บ้าน', 'Pi-hole', 'NAS'. Don't use for: enterprise campus/WAN/DC (use network-architect), BGP/IOS-specific concerns (use kbg:network-bgp-diagnostics or kbg:cisco-ios-patterns), or production business networks (use networking-engineer)."
---
