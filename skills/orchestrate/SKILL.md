...
4. **Clamp at the dispatch boundary, not the prompt (Workflow tool).** Telling the LLM "produce 16 items" is not enforcement — it's a request. Your dispatch code (clamping the work-list to the cap before spawning) is the enforcement point for a scripted worklist — doctrine, not preference, because the next Workflow author will write the same soft cap again unless the hard cap is a number in code. On the Agent tool there is no dispatch code to insert this into; the lead clamping its own per-turn dispatch count is the equivalent discipline.
