...
- **pnpm move** — Evidence: no deadline stated, but "should we" is a genuine open decision, not busywork. Alternative rejected: routing straight to `kbg:decide` — the trade-off data doesn't exist yet, so `research` has to generate it first. Revisit trigger: once the research brief lands, this re-enters triage at `kbg:decide`, not back through this matrix. Falsifying fact: a stated deadline this blocks would promote it out of "not urgent."
