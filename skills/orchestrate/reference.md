...
Agent fleet → domain (the 12-agent survivor set): `code-reviewer` (review diff — carries the comment-accuracy, type-design/illegal-states, and behavioral test-coverage lenses) · `security-reviewer` (auth/secrets/OWASP) · `silent-failure-hunter` (error-handling audit) · `build-error-resolver` (build/typecheck minimal-diff fixes — auto-detects the build system, no architecture changes) · `code-architect` (new design/system design) · `spec-miner` (extract Requirement+Invariant blocks from brownfield codebases) · `refactor-cleaner` (dead code/duplicates via knip+depcheck+ts-prune) · `ideate-critic` (fresh-context critic for `/ideate`; skill-invoked, see below) · `typescript-reviewer` (TS narrowing/`any`/strict drift) · `python-reviewer` (mutability/GIL/generators/imports) · `performance-optimizer` (bottlenecks/bundle size/memory leaks/render issues) · `chief-of-staff` (email/Slack/LINE/Messenger triage + 4-tier classification + draft replies).
