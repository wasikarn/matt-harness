...
**No numeric scoring.** Value×Risk is intentionally a binary classifier (high/low), not a weighted decision matrix with 1–5 scores. Numeric scores introduce false precision and weight-manipulation risk. If N≥3 alternatives need ranking, use `/adr` with a Pugh Matrix (+/S/-) instead of a numeric grid.
...
| **tournament** | N agents attempt the same task via different approaches; pairwise judges pick a winner. (`/adr` Pugh Matrix is the static analog.) | self-preferential-bias (model + approach diversity) |
