...
Agent fleet → domain: `security-reviewer` (auth/secrets/OWASP) · `backend-engineer` (API/data/migration/clarity-only simplification) · `frontend-engineer` (UI/a11y/state) · `devops-engineer` (CI/CD/deploy/IaC) · `test-engineer` (tests) · `researcher` (investigation/unknowns) · `code-reviewer` (review diff) · `code-architect` (new design) · `Explore` (understand existing code) · `comment-analyzer` (comment/docstring accuracy) · `pr-test-analyzer` (PR test coverage) · `silent-failure-hunter` (error-handling audit) · `typescript-reviewer` (TS narrowing/`any`/strict drift) · `python-reviewer` (mutability/GIL/generators/imports) · `go-reviewer` (`%w`/`ctx`/goroutines/channels) · `rust-reviewer` (ownership/lifetimes/`unsafe`/idiomatic Rust) · `react-reviewer` (hooks/render perf/server-client boundary/a11y/React-specific security) · `vue-reviewer` (Composition API reactivity pitfalls/component architecture/template security/performance) · `django-reviewer` (ORM/DRF/migration safety/security misconfig) · `fastapi-reviewer` (async correctness/dependency injection/Pydantic/OpenAPI quality) · `healthcare-reviewer` (clinical safety/CDSS accuracy/PHI/EMR-EHR integrity) · `build-error-resolver` (build/typecheck minimal-diff fixes — no architecture changes) · `cpp-build-resolver` (C++ build/CMake/linker/template errors, minimal diff) · `cpp-reviewer` (memory safety/modern idioms/concurrency/performance) · `dart-build-resolver` (dart analyze/compilation/pub conflicts, minimal diff) · `flutter-reviewer` (widget best practices/state management/Dart idioms/perf/a11y) · `go-build-resolver` (go build/vet/linter warnings, minimal diff) · `java-build-resolver` (Maven/Gradle compiler/dependency errors for Spring Boot/Quarkus, minimal diff) · `java-reviewer` (layered architecture/JPA-Panache/MongoDB/security/concurrency) · `kotlin-build-resolver` (Kotlin compiler/Gradle errors, minimal diff) · `kotlin-reviewer` (idiomatic Kotlin/coroutines/Compose/Android pitfalls) · `react-build-resolver` (Vite/webpack/Next.js/CRA/Bun build/hydration/bundler config) · `rust-build-resolver` (cargo/borrow checker/Cargo.toml errors, minimal diff) · `swift-build-resolver` (Xcode/SPM/code-signing errors, minimal diff) · `swift-reviewer` (protocol-oriented design/value semantics/ARC/Swift Concurrency/idiomatic) · `database-reviewer` (Postgres query/schema/security/Supabase) · `django-build-resolver` (pip/Poetry/migration/import errors, minimal diff) · `mle-reviewer` (data contracts/feature pipelines/training reproducibility/model serving/monitoring) · `a11y-architect` (WCAG 2.2/inclusive design/accessibility audit) · `performance-optimizer` (bottlenecks/bundle size/memory leaks/render issues) · `tdd-guide` (write-tests-first red-green-refactor / 80%+ coverage enforcement) · `code-simplifier` (clarity-consistency-maintainability refactor, behavior-preserving) · `refactor-cleaner` (dead code/duplicates via knip+depcheck+ts-prune) · `e2e-runner` (Playwright end-to-end / flaky quarantine / screenshots+traces) · `planner` (complex features/architectural changes/refactor blueprints) · `architect` (system design/scalability/architectural decisions) · `harness-optimizer` (local harness reliability/cost/throughput tuning) · `loop-operator` (autonomous-loop monitoring/safe intervention on stalls) · `agent-evaluator` (external 5-axis rubric scoring of another agent's output — accuracy/completeness/clarity/actionability/conciseness with VERDICT) · `chief-of-staff` (email/Slack/LINE/Messenger triage + 4-tier classification + draft replies) · `spec-miner` (extract Requirement+Invariant blocks from brownfield codebases) · `doc-updater` (codemaps + documentation; runs /update-codemaps + /update-docs) · `docs-lookup` (spawnable fresh-context Context7 docs lookup via mcp__context7__*) · `conversation-analyzer` (analyze transcripts for hookable behaviors; auto-fired by /hookify).

Domain specialists (dispatch when the task matches the domain — `backend-engineer`/`frontend-engineer` are the generalist fallback): `data-engineer` (ETL/warehouse/streaming pipelines) · `ml-engineer` (model serving/inference/feature stores) · `mobile-engineer` (iOS/Android/React Native) · `platform-engineer` (service mesh/gRPC/inter-service resilience) · `maintenance-engineer` (legacy/deprecation/tech-debt) · `type-design-analyzer` (type/DTO/schema/invariant review) · `ux-reviewer` (UX/a11y/WCAG/interaction flow) · `product-analyst` (requirements/scoping/acceptance criteria) · `technical-writer` (READMEs/runbooks/ADRs) · `api-doc-specialist` (OpenAPI/SDK/contract docs) · `i18n-specialist` (localization/RTL/locale formatting) · `compliance-engineer` (GDPR/SOC2/HIPAA controls) · `finops-engineer` (cloud cost/rightsizing) · `incident-commander` (live-incident coordination) · `infra-engineer` (physical/virtual hosts, storage, HA — NOT CI/CD, NOT routing) · `networking-engineer` (BGP/OSPF/VLAN/ACL/VPN/firewall policy — NOT hosts, NOT application protocols) · `network-architect` (read-only multi-site/campus/WAN/DC topology planner — defers detail to kbg:cisco-ios-patterns, kbg:network-bgp-diagnostics, kbg:homelab-network-setup) · `network-troubleshooter` (read-only OSI-layer diagnostic with evidence-backed root cause summary) · `marketing-engineer` (attribution/lifecycle/martech/funnels — NOT UI copy, NOT ETL) · `security-auditor` (standalone deep threat-model + remediation plan — NOT the fast flag inside `kbg:review-pr`, which is `security-reviewer`).
...
- `inferential-structural-judge` — SessionEnd sensor for diff-shape judging (over-engineering / arch-drift / test-pattern / doctrine-conformance; 4-dimension schema per `docs/research/inferential-structural-judge-design.md` §3). Invoked by `hooks/session/inferential-structural-judge-on-session-end.sh` — **currently dormant (hook unwired, script absent); re-wire is a pending follow-up.** Advisory only — journals to `~/.claude/governance-events.jsonl`, never emits `permissionDecision`.
