...
Produce a decision record at the end of any `decide` or `strategize` session. The full template
— Decision statement, Frame, tested-assumptions table, Decision block (evidence-anchored
Confidence + Flip condition), Commitment block — lives in `references/output-format.md`:
**read it before writing the record**; its qualifier paragraphs (evidence-anchor requirement on
every Confidence cell, `First reversible step` must act on the Flip condition) are what the
Completion criterion below holds the record to.
