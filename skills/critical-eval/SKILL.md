---
name: critical-eval
description: "Stress-test reasoning quality in arguments, decisions, PR descriptions, ADRs, incident write-ups, RFCs, business cases, and one-pagers. Use whenever the user asks to 'critique', 'evaluate reasoning', 'check assumptions', 'stress-test this argument', 'review this logic', 'audit the reasoning', 'does this hold up', 'is the logic sound', or 'something feels off' about a claim, proposal, or causal chain. Also trigger when a user presents an overconfident plan (e.g., 'definitely safe', 'zero downtime', 'minimal risk') with thin evidence, or asks you to sanity-check a decision before they present it to leadership. Do NOT use for: exploring system dynamics (use /probe), reviewing code implementation (use /review-pr), security audits (use /security-auditor), or pure research (use /research-brief)."
---
