...
**Named models** (cc-thinking-skills): profile → find the one bottleneck → fix that is *theory-of-constraints*; the ">50% improvement or discard" rule is the *scientific method*'s falsification gate. Catalog + honesty caveat: read via Bash with `cat "${KBG_PLUGIN_ROOT}/docs/reference/reasoning-models.md"`.
