...
- **Fork timeout fallback.** Some phases invoke `context: fork` skills that spawn agents. If the agent hangs or stalls for >2 minutes, cancel it and invoke the phase's underlying command directly (e.g., `/fix-bug` instead of waiting on Phase 2 via `/ship-change`).
...
| 3 | Self-review via `/review-pr` (zero Critical findings gate) | → 4 |
...
| 2 → 3 | `/review-pr` | After implementation done |
| 3 → 4 | Open PR; `/address-review` if comments arrive | After zero Critical findings |
| 4 → 3 | `/review-pr` again | After `/address-review` fixes |
