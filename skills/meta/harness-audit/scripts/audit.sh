...
# ── checks (sourced in filename-glob order; fragments are numbered 1..57 with
# filename prefix == '# N.' header, so glob-sorting by 01..57 sources them in
...
# and exit (skipping the full loop + the 57-fragment integrity guard, which
...
# Split-integrity guard: exactly 57 fragments, each carrying one '# N.' header,
# numbers 1..57 each exactly once. Source scope == count scope == the SAME glob
