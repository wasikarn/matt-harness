...
# Repointed from commands/ship-merge/COMMAND.md to skills/workflow/ship-merge/SKILL.md
# 2026-08-25 (#103, commands→skills convergence, spec #101) — the surface
# moved, the risk and the enforcement mechanism did not.
_f="$CLAUDE_DIR/skills/workflow/ship-merge/SKILL.md"
...
if [ -f "$_f" ]; then
  ...
  crit "ship-merge skill not found at skills/workflow/ship-merge/SKILL.md — cannot verify the disable-model-invocation flag that is the only mechanism blocking the model from self-invoking a server-side PR merge"
fi
