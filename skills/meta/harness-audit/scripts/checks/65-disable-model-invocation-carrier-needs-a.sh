...
# Checks 36/40/45/58-64 hardcode one CRIT check per KNOWN carrier — a
...
# per-skill check doesn't self-extend: an 11th disable-model-invocation
...
# skill, exactly the gap 58-64 just closed for the 7 that existed at
# audit-write-time. Found by the 2026-08-30 deep-audit's advisor pass on the
# checks-58-64 commit itself — not hypothetical: this repo added wiki-ingest,
# idea-scan, and restored compliance-audit within the same few weeks.
#
...
# all 10 current ones use; a guard written under a different variable name
...
for f in "$CLAUDE_DIR/skills"/*/SKILL.md "$CLAUDE_DIR/skills"/*/*/SKILL.md; do
  ...
  if ! printf '%s\n' "$_guarded_paths" | grep -qxF "$_rel"; then
    ...
    warn "'$_nm': disable-model-invocation: true with no dedicated CRIT-guard check file referencing '$_rel' — extend the checks-36/40/45/58-64 pattern (checks/58-64 are the template) so a future silent flag-drop on this skill is caught, not just relying on checks 05/47's dodgeable description-shape backstop"
  fi
done
