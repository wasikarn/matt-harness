...
# independence maker≠checker depends on. Load-bearing at L3 (CLAUDE.md's "The operating model (was L3 bounded autonomy, retired)" section) — these agents
...
for f in "$CLAUDE_DIR/agents"/*.md; do
  ...
  bad="${bad# }"
  if [ -n "$bad" ]; then
    crit "agent '$name' is a reviewer but grants '$bad' — read-only invariant (maker≠checker) broken; reviewers must not mutate what they review (CLAUDE.md's \"The operating model (was L3 bounded autonomy, retired)\" section, L3 evolution)"
  fi
done
