...
for f in "$CLAUDE_DIR/commands"/*.md "$CLAUDE_DIR/commands"/*/COMMAND.md "$CLAUDE_DIR/skills"/*/SKILL.md "$CLAUDE_DIR/skills"/*/*/SKILL.md; do
  [ -f "$f" ] || continue
  case "$f" in */skills/_*) continue ;; esac
  head -20 "$f" | grep -qF 'disable-model-invocation: true' || continue
  _nm=$(basename "$f" .md); case "$f" in */SKILL.md|*/COMMAND.md) _nm=$(basename "$(dirname "$f")") ;; esac
