...
- **Not the primary writer — Claude Code's native auto-memory is.** This skill adds the one thing the ambient path structurally cannot: the retrospective, whole-transcript sweep for **cross-turn** patterns.
- **Operator-invoked, gated per batch, no flag by design.** Every proposed candidate passes an `AskUserQuestion` gate (reject = nothing written); the in-flow gate, not a user-only lockout, is the safety. Full posture (native-path facts, retired SessionEnd design, `learn-nudge.sh`'s role, contrast with recursive-improve): read `references/autonomy-posture.md`.
