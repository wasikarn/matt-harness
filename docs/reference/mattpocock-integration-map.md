...
| codebase-design | model | routed — `docs/reference/decision-doctrine-map.md`'s "plan authoring" row names it for deep-module vocabulary mid-blueprint; `agents/code-architect.md` and `agents/backend-architect.md` boundary lines point to it for the same reason; kbg's native design agents are `mh:code-architect` / `mh:backend-architect` for the blueprint itself |
...
| improve-codebase-architecture | user | `docs/agents/domain.md` (path into domain-modeling); `docs/reference/decision-doctrine-map.md`'s "plan authoring" row names it for a whole-repo architecture pass; see Reverse handoffs §3 |
