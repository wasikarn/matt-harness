...
| `hands-off-to` | One skill's scope ends and names the skill that owns the next piece | The "Boundary with X" prose sections — currently only 2 in the whole fleet, both in `skills/workflow/orchestrate/routing.md` (boundary with the decision doctrine, METHODOLOGY Rule 1; boundary with `/mattpocock-skills:wayfinder`, user-invoked) | Prompt-discipline only — this edge exists purely as prose a human/model reads, not a structure anything queries or validates. Sparse: worth naming as under-instrumented rather than assuming it's systematic. |
