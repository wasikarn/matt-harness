...
| `hands-off-to` | One skill's scope ends and names the skill that owns the next piece | The "Boundary with X" prose sections — currently only 2 in the whole fleet, both in `skills/orchestrate/SKILL.md` (lines 340–347 boundary with `kbg:decide`; lines 349–358 boundary with `mattpocock-skills:wayfinder`) | Prompt-discipline only — this edge exists purely as prose a human/model reads, not a structure anything queries or validates. Sparse: worth naming as under-instrumented rather than assuming it's systematic. |
