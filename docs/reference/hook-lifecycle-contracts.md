...
| UserPromptSubmit (*) | `flow-nudge.sh` | FF / inferential | Advisory: when the prompt looks like non-trivial engineering work, nudge `mattpocock-skills:grilling` → `mattpocock-skills:to-spec` → `mattpocock-skills:to-tickets` → `/ship`. Never blocks. |
