...
| PreToolUse (Write\|Edit\|NotebookEdit) | `main-write-budget.sh` | FF / computational | Ask-tier only — never denies. Nudges main to consider dispatching once its own cumulative inline Write/Edit/NotebookEdit count for the session crosses an operator-set budget. Opt-in and OFF by default: no-op unless `MH_MAIN_WRITE_BUDGET` is a positive integer. **Staleness ceiling:** `main_writes` is re-derived from the transcript once per Stop, not per tool call, so the count read here is always as of the last completed turn, never the in-progress one. **Latch, by design:** once the budget is crossed, this asks on every subsequent main write for the rest of the session (silence it with Claude Code's own session-wide "always allow"). Threshold is provisional — the suggested starting value (150) is not derived from enough data; see the gate's own header comment for the current sample. Subagent writes (`agent_id` present) are unaffected. No-ops entirely, unconditionally, when `MH_GUARDED_WORKSPACE` is also set — see `docs/reference/env-vars.md`'s Main-write-budget gate section for why. |
