...
|---|---|---|
| `MAX_THINKING_TOKENS` | `0` to disable, otherwise leave unset | Fixed extended-thinking token budget — but on this harness's actual model (Sonnet 5), it does nothing: adaptive reasoning "has no effect on Fable 5, Sonnet 5, or Opus 4.7 and later, which always use adaptive reasoning" (confirmed against `code.claude.com/docs/en/env-vars`, 2026-08-20). No numeric live default is documented; the prior `10000` default/raise-it advice here was unsourced. `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING` can't restore the fixed-budget behavior on this model either. Set to `0` to disable thinking entirely for trivial tasks — that part still holds. |
