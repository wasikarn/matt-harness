...
| `plugin:atlassian:atlassian` | Jira + Confluence + Compass; JQL/CQL search, issue create/edit/link, page create/update, worklog, transitions | use the tool schemas directly (`mcp__*atlassian*`) — no dedicated kbg contract doc exists today |
| `plugin:context7:context7` | Current library/framework/SDK/CLI docs — prefer over web search for library docs (training data lags releases) | `resolve-library-id` → `query-docs`; cap 3 calls/question (past 3, state uncertainty — don't loop); among `resolve-library-id` results pick by benchmark score > source reputation > version-match (prefer a version-specific `/org/project/vX.Y.Z` ID if the user named a version); redact secrets from any query |
