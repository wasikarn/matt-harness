...
| #32 | `worktree-isolation-parallel-agents` | Vendor primitive (worktree lifecycle is owned by `claude-code`, not by the plugin) | F8.5 bounded-fan-out gate in `commands/team-build.md` Step 6 (16 agents per wave, queue the rest) + per-wave contract chain in `skills/orchestrate/SKILL.md` § Bounded fan-out. Surfaced as `f8_5_overflow_warnings` in `scripts/orchestrate-dispatch.py` |
