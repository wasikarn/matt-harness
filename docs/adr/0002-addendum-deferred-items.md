...
| #35 | `mcp-connectors-act-in-real-tools` | Vendor primitive (MCP transport + connector registration is host-owned) | `hooks/gates/db-write-gate.sh` gates the one cross-vendor path the plugin owns (DB writes); other MCP connectors (atlassian, qmd, code-review-graph) inherit the vendor's reachability model |
...
| #47 | `durable-checkpointed-state-recovery` | L4 territory (auto-recovery from a crashed session is unattended self-repair) | Journaled events in `.scratch/<slug>/` (session-scoped) + `docs/harness-decay-cadence.md` § Permission re-audit; cross-machine state continuity is out-of-scope by design (per [ADR 0002](0002-autonomy-invariant.md) §Context and §Rejected alternatives) |
