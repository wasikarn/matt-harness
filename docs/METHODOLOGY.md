...
- **This fleet's delegation mechanisms are all brakes, not an accelerator (prose-only)** — the fan-out cap, `AskUserQuestion` friction on write-capable dispatch, and `agent-recursion-guard.sh` all stop *over*-delegating; none of them push back when the orchestrator is hoarding work in the main thread instead. No check currently measures or nudges toward more delegation, only away from too much of it (`skills/workflow/orchestrate/SKILL.md`'s own note on this gap has the follow-up). Raising delegation rate alone, without also raising scoping quality, has made results worse elsewhere (Claude Code issue #40339) — don't treat "delegate more" as a fix on its own.
