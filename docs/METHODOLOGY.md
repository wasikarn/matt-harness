...
- **This fleet's delegation mechanisms were all brakes, not an accelerator, until GH #120.** The fan-out cap, `AskUserQuestion` friction on write-capable dispatch, and `agent-recursion-guard.sh` still only stop *over*-delegating. The accelerator is `hooks/advisory/flow-nudge.sh`'s delegation-ratio trigger (enforced: the hook fires deterministically on its own trigger, independent of the IMPL-verb-gated plan-mode heredoc it used to live inside) — a prompt naming >~3 files, or a files-plural noun co-occurring with a breadth word, fires it whether or not the prompt also carries an IMPL verb, so a read/research-heavy prompt with no impl verb ("review these 5 files...") now gets it too. The nudge names the session's actual orchestrator:subagent token ratio (derived from `hooks/stop/cost-tracker.sh`'s `costs.jsonl`, dedup'd latest-per-`(stream, model, agent_type)` key — those rows are cumulative-to-date snapshots, not deltas) and points at the F9 spawn-prompt template, not a bare "delegate more" line — raising delegation rate alone, without also raising scoping quality, has made results worse elsewhere (Claude Code issue #40339). What's still prose-only: whether the model actually acts on the nudge — same as the plan-mode nudge it lives beside, this is advisory, not a gate.
