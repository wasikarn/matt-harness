...
- **A repo-wide "all output must be verdict-first" convention or lint check** —
  ...
  `agents/plan-reviewer.md:183`'s mid-block verdict placement; `agents/requirement-analyst.md:139`
  is already verdict-first and wouldn't move — see `agent-authoring-conventions.md` item 9's
  scope note).
...
**Correction (2026-09-21, `mh:deep-audit` round 1, fresh-context Codex checker on `d2e80b52`,
shipped as `bc5c5381`):** the artifact had committed one more overclaim past the two the
plan-review round caught — the Verdict paragraph, row 6, and "Deliberately not shipped" all
counted `agents/plan-reviewer.md` as a 5th verdict-last agent. It isn't: `plan-reviewer.md:183`'s
`verdict:` sits mid-block with 4 fields after it, a structured multi-field report, not this
repo's flat verdict-last shape (now scoped out explicitly in `agent-authoring-conventions.md`
item 9). The real cohort is 4 agents. Also: row 8's `doc-template.md` line citations were stale
by 2 lines against `d2e80b52`'s own template edit, and the artifact lacked the `**Score:**`
header line its own fix mandates — added. All three spots were edited in place above rather
than left wrong with a note here; that is a stated exception to this slot's append-only rule,
and `git show bc5c5381` is the record of what changed.

**Correction (2026-09-21, `mh:deep-audit` round 2, fresh-context Codex checker on
`bc5c5381`):** round 1's fix shipped one new error and left four consistency defects. New error:
"Deliberately not shipped" said a blanket verdict-first rule would break
`agents/requirement-analyst.md:139`'s placement — it is already verdict-first and wouldn't move;
only `plan-reviewer.md` would. Defects: the round-1 correction note sat at the top of the file
instead of this reserved slot (`doc-template.md:62-63`) and named 2 of the 3 spots it changed;
row 4's `` `(^|\n)` `` had an unescaped pipe that split the table row into 7 cells, pushing its
`PARTIAL` out of the Verdict column (pre-existing since `d2e80b52`); row 8's pre-fix citation
carried no revision pin. All edited in place above under the same stated exception;
`git log -2 -- <this file>` is the record.
