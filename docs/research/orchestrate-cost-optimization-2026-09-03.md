...
6. Cache signature after each diff, on a ≥30-turn session: orchestrator rows in `costs.jsonl` show `cache_write_tokens / turns` ≤ 1.5× the pre-diff median and `cache_read_per_turn` down by ≥ (bytes removed ÷ 4) − 500. **Correction (2026-09-25, `mh:deep-audit` on issue #162's fix, verified against `hooks/stop/cost-tracker.sh`):** since 1.1.118, `cache_write_tokens` on a row is 5-minute-TTL cache writes only, not every cache write — a new `cache_write_tokens_1h` field carries the 1-hour-TTL subset separately (measured live on 2026-09-25: ~99% of real cache-write tokens on this machine are 1-hour-TTL). Replaying this criterion literally against a post-1.1.118 session would compare `cache_write_tokens / turns` against a pre-1.1.118 median computed from the old, undivided field — an apples-to-oranges comparison that would read as a near-total cache-write drop regardless of actual cache growth. This criterion predates the split and was never re-run against it; doesn't change this round's own recorded outcome, only a future literal replay.
