...
**Why runtime-only as the default?** — The harness's autonomy invariant (the no-model-self-start rule in METHODOLOGY.md and CLAUDE.md §The operating model) forbids autonomous mutation. A runtime toggle is human-gated by definition — the operator edits a JSON file and the change is effective immediately. A structural removal requires 5 file edits and a version bump, which is the right shape for "this sensor is genuinely dead" but overkill for "I want to silence this for a week."
...
- **CI cron (`.github/workflows/sensor-staleness-notify.yml`).** — Rejected. A cron is **L3 autonomy** per `METHODOLOGY.md Rule 8 + CLAUDE.md §The operating model` ("no CronCreate, no `/loop`, no scheduled unattended loop"). The kbg harness is personal — sessions *are* the cadence. A SessionStart hook fires when the operator picks up the harness, which is the right beat; a cron fires on a wall clock, which the operator may not be at.
