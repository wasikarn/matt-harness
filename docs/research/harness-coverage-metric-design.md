...
```jsonc
{
  ...
  "grid": [
    ...
    },
    {
      ...
      "intentional_gap": {
        "rationale": "Böckeler L465–478 names behaviour-inferential-FB as the unsolved elephant; kbg's verification-gate.sh / fabrication-verdict-log.sh / inferential-structural-judge are advisory-only by ADR 0002 §L115 (LLM-judge-circularity mitigation).",
...
1. **`inf-fb-behaviour`** (cell_id: `inf-fb-behaviour`) — Böckeler L465–478 names this as the article's deepest open problem; kbg's `verification-gate.sh` / `fabrication-verdict-log.sh` / `inferential-structural-judge` are all advisory-only per ADR 0002 §L115 (LLM-judge-circularity). Revisit when: a different model class is available for judging, OR a non-model judge (mutation testing, property-based tests) lands for behaviour. **This is the textbook "article-punted" gap.**
...
- **SKILL-1 (skill author):** the exact markdown table layout for the human render (axis-rows × direction-columns is the §2 shape, but the skill can add a "Top 3 cells to fix" header summary); whether to extend `skills/harness-health/SKILL.md` or author a new `skills/harness-coverage/SKILL.md` (per the plan's SKILL-1 row, either is in scope); the audit #31.1 canonical sections (Input Contract / Output Format / Failure Modes).
