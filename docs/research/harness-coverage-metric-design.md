...
The output is a 12-cell scorecard with a global rollup + 30-session drift line, rendered as a markdown table by the future `kbg:harness-coverage` skill. The metric is **deterministic** — it counts events, parses JSON, and computes arithmetic. It does **not** call a model. The *interpretation* of a 0% cell (gap vs feature) is human; the metric surfaces the question, the operator answers it.
...
| Axis \\ Direction | comp-ff | comp-fb | inf-ff | inf-fb |
|---|---|---|---|---|
| **maintainability** | populated (8 PreToolUse gates) | populated (post-edit-audit, security-diff-review, 204 critical-hooks tests, 38 audit checks) | populated (doctrine-bootstrap, iron-rule-reminder, orchestrator-nudge, 34 skill descriptions) | populated (kbg:review-pr, kbg:code-reviewer — advisory only) |
| **arch-fit** | populated (JOURNAL-SCHEMA.md, 14-event hook surface, 1536-char description budget) | populated (BOUNDARY.md auto-regen, last_permission_review cadence) | populated (token budgets via METHODOLOGY, kbg:perf skill) | **coverage hole** — no inferential arch-review agent |
| **behaviour** | populated (ACCEPTANCE.md → run-acceptance) | populated (eval/run-eval.py --gate, run-acceptance 5-state) | partial (pre-ship-verify command) | **intentional gap** — kbg's `verification-gate.sh` precedent is advisory-only by design; `inferential-structural-judge` (design per sibling plan) is the same pattern |

The `arch-fit` × `inf-fb` cell is a **coverage hole** (no sensor exists; would be a future `diff-arch-drift-judge` agent). The `behaviour` × `inf-fb` cell is an **intentional gap** (Böckeler L465–478 punts; the kbg pattern is to journal verdicts and let the operator interpret, not block). The distinction between the two is the §3 annotation.

## 3. Cell schema (the data contract)

This is the JSON shape the Wave-2 backend-engineer (`AGG-1`) will read as the data contract for `scripts/harness-coverage.py`. The aggregator emits one entry per cell, plus a global rollup. The schema is intentionally **deterministic and self-describing** — every field is either a count, a percentage, or a fixed-enum annotation. No model in the loop.
...
```jsonc
{
  ...
  "grid": [
    ...
    },
    {
      ...
      "intentional_gap": {
        "rationale": "Böckeler L465–478 names behaviour-inferential-FB as the unsolved elephant; kbg's verification-gate.sh / fabrication-verdict-log.sh / inferential-structural-judge are advisory-only by ADR 0002 §L112 (LLM-judge-circularity mitigation).",
...
1. **`inf-fb-behaviour`** (cell_id: `inf-fb-behaviour`) — Böckeler L465–478 names this as the article's deepest open problem; kbg's `verification-gate.sh` / `fabrication-verdict-log.sh` / `inferential-structural-judge` are all advisory-only per ADR 0002 §L112 (LLM-judge-circularity). Revisit when: a different model class is available for judging, OR a non-model judge (mutation testing, property-based tests) lands for behaviour. **This is the textbook "article-punted" gap.**
...
A coverage number without a remediation path is a vanity metric. The 3-step loop the operator runs when the quarterly review (`kbg:harness-coverage` via `decay-cadence`) flags a `coverage_hole` or `stale` cell:
...
3. **The follow-up plan's `AGG-1` row** is a new commit to `hooks/sensors.json` (or a future registry extension) **and** a one-line update to the aggregator's cell-enumeration table. The next `kbg:harness-coverage` run will then score the cell as `populated` (or, if the annotation is revised, as `intentional_gap` with the new rationale). The metric is *self-updating through the operator's normal planning workflow* — no separate "metric update" step is needed.

The loop is **human-gated at step 2** (per ADR 0002 / autonomy invariant). The metric does not open follow-up plans itself; it surfaces the question. This is the symmetric counterpart of `verification-gate.sh`'s "advisory only" stance: the metric is a *sensor*, not a *gate*. The autonomy invariant forbids cron / unattended mutation, so the "loop" is operator-driven, with `decay-cadence` as the cadence.
...
| Q7 | Is this a `kbg:` skill or a script? | **Resolved** | §1 + §3 (the script `scripts/harness-coverage.py` emits the JSON in §3; the future `kbg:harness-coverage` skill renders it) |
...
- **SKILL-1 (skill author):** the exact markdown table layout for the human render (axis-rows × direction-columns is the §2 shape, but the skill can add a "Top 3 cells to fix" header summary); whether to extend `skills/state-of-the-harness/SKILL.md` or author a new `skills/harness-coverage/SKILL.md` (per the plan's SKILL-1 row, either is in scope); the audit #31.1 canonical sections (Input Contract / Output Format / Failure Modes).
...
- **INT-1 (integration smoke):** the manual smoke procedure (run a session that touches ≥ 1 file, observe the `kbg:harness-coverage` render shows 12 cells + global + drift). The smoke does *not* require the registry to be live; the aggregator runs against an empty registry and emits 12 cells with `status: "coverage_hole"`, which is the correct first-run state.
