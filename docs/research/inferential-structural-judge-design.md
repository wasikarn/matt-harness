...
|---|---|---|
| **1-3** | `accept` | Journal only. The session is silently green. `kbg:state-of-the-harness` shows the verdict count, never the rationale. |
| **4-6** | `flag` | Journal + the verdict surfaces in `kbg:state-of-the-harness` (or the new `kbg:harness-health` command) with the rationale. The operator is *informed*, not interrupted. |
...
**(c) No mutation, no `permissionDecision`.** The agent **never** writes to the repo and **never** emits a `permissionDecision`. The verdict is journal-only; the future `kbg:state-of-the-harness` / `kbg:harness-health` surface is a *read* of the journal, not a write-back. This is the autonomy invariant (ADR 0002 §L112: *"Pure SENSOR: it journals but NEVER emits a permissionDecision"*, the `verification-gate.sh` precedent). It is the load-bearing guard against the L4-loop covert failure mode Böckeler's L356–359 warning points at: an inferential-FB sensor with `permissionDecision: deny` authority is a model-driven mutation gate, which the invariant forecloses. CI enforcement: `git grep -n 'permissionDecision' agents/inferential-structural-judge.md hooks/inferential-structural-judge-on-session-end.sh` must return zero matches (the plan's final validation command).
