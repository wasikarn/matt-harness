...
- **Session budget gate:** the harness allocates 30,000 tokens per session for this class of budget (a doctrine point from when this design was written — current `docs/METHODOLOGY.md` carries only Rules 1, 2, 3, 4, 13, 14, and no live rule covers session token budgets; this citation is historical context for the design, not a pointer to current doctrine). The new agent **must** run within that budget, not add a per-call budget. Concretely: the SessionEnd hook reads the session's running token count (via the same mechanism `verification-gate.sh` already uses to read `.scratch/*/verification-trail.md`); if the count is already ≥ 25,000, the hook **skips** and journals a `skipped: budget` event with the same envelope shape and `event: "inferential_structural_verdict_skipped"`.
