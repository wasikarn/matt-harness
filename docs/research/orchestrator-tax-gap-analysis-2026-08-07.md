...
|---|---|---|---|
| G1 | Route mechanical subagent work to a cheaper model | **Declined for now — measurement gap closed same day** | On operator follow-up, the `kbg:cost-aware-llm-pipeline` citation turned out wrong (it's scoped to the user's own API-calling code, not to `orchestrate`'s own dispatch), and the "contradicts the tool contract" framing was overstated (the Agent tool's `model` param explicitly allows override "when highly confident a different tier fits" — it doesn't forbid this, it asks for a reason). The real blocker was that `cost-tracker.sh` had no way to tell whether any agent type was actually burning frontier-tier cost on mechanical work. Fixed 2026-08-07: `cost-tracker.sh`/`cost-report.md` now break subagent spend down by `agent_type` (read from each subagent's `agent-<id>.meta.json` sibling), including a "By agent type" report section. Still no model-routing *rule* — that's a decision for whoever reads real `agent_type` numbers once they exist, not something to write blind. |
