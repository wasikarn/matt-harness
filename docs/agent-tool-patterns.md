...
- **F1 Bash-gate pattern (retired)** — `hooks/gates/validator-bash-guard.sh` ran a deny-pattern check on the then-14 validator-class agents through v0.4.18; deleted in the v0.6.0 reset and not rebuilt (see point 4 above). Today's 4 validator-class agents (`code-architect`, `security-reviewer`, `silent-failure-hunter`, `typescript-reviewer`) have no runtime enforcement beyond the allowlist's `Bash` grant and prompt doctrine.
