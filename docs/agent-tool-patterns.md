...
- **The no-model-self-start rule** (`CLAUDE.md`'s Operating model, under §Architecture) — the autonomy invariant is enforced by **allowlist-based tool grants** (Pillar 1: deterministic via `audit.sh #32` on `recursive-improve`'s `disable-model-invocation: true`). The allowlist convention is the substrate for the invariant's enforcement. (See `CLAUDE.md`'s Operating model, under §Architecture, for the invariant's prose form.)
