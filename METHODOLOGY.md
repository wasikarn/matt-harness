...
When a loop *verifies*, its stop-signal must reduce to an objective check — a test result, an exit code, a fresh-context adversarial pass — never the implementer agreeing with its own work; the verifier gets fresh context, not the implementer's transcript. (This scopes only the *verification* signal; it does not loosen the harness's human approval gates.) Named corollary: a working loop whose output the human hasn't personally read is **comprehension debt at compound interest** — the gap between what the repo contains and what the operator understands widens with every merged PR; accepting that output without forming an opinion is **cognitive surrender**. The autonomy invariant guards both: every loop terminates at a human gate, and the doctrine is the gate. (Under **L3 bounded autonomy** — opt-in `KBG_AUTONOMY_L3=1`, default OFF, [ADR 0003](docs/adr/0003-l3-bounded-autonomy.md) — that gate is the **pre-push review**, not per-mutation: the loop runs bounded cycles unattended, commits local-only, and stops at human review before any push. It still cannot self-start; its in-loop check is the computational gauntlet, never a model-as-gate.)
