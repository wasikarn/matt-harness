...
- **One-shot subagent must be torn down by its parent.** If you spawn a subagent for a bounded read-only pass (blueprint, audit, research map) and consume its output inline, stop it before you continue — backgrounded agents do not self-terminate. The parent session owns teardown for any ad-hoc `Agent` spawn that is not part of an `orchestrate` dispatch flow. Leaving it idle creates a "Background work is running" blocker on the next exit.
