...
# Advisory: nudge /grilling → /to-prd → /to-issues → /ship-task when the
...
if ! printf '%s' "$prompt" | /usr/bin/grep -qiE '\b(implement|build a feature|refactor|redesign|migrate|architect|new (endpoint|command|skill|surface|hook|agent)|grill[- ]|to-prd|to-issues|ship-task)\b'; then
  exit 0
fi
...
[kbg:flow-nudge] Non-trivial work detected — consider matt's flow:
  /grilling → /to-prd → /to-issues → /ship-task
