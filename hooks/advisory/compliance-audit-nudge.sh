...
# remind the model to tell the user that /kbg:compliance-audit exists --
# never to dispatch it (commands/compliance-audit.md is
...
# commit is a much closer analog to ExitPlanMode's cleanliness -- a real,
# deterministic, low-noise "this unit of work is considered done" signal.
#
...
# with a per-line try/except (cost-tracker.sh's own resilience pattern),
# confirming the precise structural signal -- an assistant-role entry whose
# message.content[] carries a tool_use block with name=="ExitPlanMode" and
# a non-empty input.plan. Measured against a real 79MB/27,125-line
...
nudge = (
    "[kbg:compliance-audit-nudge] A git commit landed after a plan was "
    ...
    "tell the user they can run /kbg:compliance-audit themselves to "
