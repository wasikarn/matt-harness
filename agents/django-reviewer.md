...
description: "Expert Django code reviewer — ORM correctness, DRF patterns, migration safety, security misconfigurations, production-grade Django practices. Use for all Django code changes (MUST BE USED for Django projects) touching settings.py, models.py, migrations/, views.py, urls.py, or DRF serializers/views. Use when reviewing Django settings, models, migrations, DRF views/serializers, or auth/middleware. Thai: 'review Django', 'Django review', 'DRF', 'ตรวจ Django', 'โค้ด Django'. Don't use for: Flask/FastAPI (use fastapi-reviewer), raw SQLAlchemy without Django ORM, or non-Django Python (use python-reviewer). Read-only."
