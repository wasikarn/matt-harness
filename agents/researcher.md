...
- **Wide sweep (>5 articles / >3 libraries for one question):** spawn a `general-purpose` layer-2 agent per cluster (e.g. one for "Claude Code plugins", one for "hook contracts", one for "vendor hook model"). Each returns a structured brief: claims + sources + verdict. You synthesize the verdicts, not the raw fetches.
