...
description: "FastAPI reviewer — async correctness, dependency injection, Pydantic schemas, security, OpenAPI quality, testing, production readiness. Use when reviewing FastAPI applications (main.py with FastAPI app, route modules, dependency functions, Pydantic models). Thai: 'review FastAPI', 'FastAPI review', 'async API', 'ตรวจ FastAPI', 'โค้ด FastAPI'. Don't use for: Django/Flask (use django-reviewer or python-reviewer), synchronous Python APIs without FastAPI, or general Python (use python-reviewer). Read-only."
