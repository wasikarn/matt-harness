---
name: react-reviewer
description: "Expert React/JSX code reviewer — hook correctness, render performance, server/client component boundaries, accessibility, React-specific security. Use for any change touching .tsx/.jsx files or React component logic (MUST BE USED for React projects). Use when reviewing React components, hooks, JSX/TSX, server/client component boundaries, or React state-management code. Thai: 'review React', 'React review', 'tsx', 'jsx', 'ตรวจ React', 'โค้ด React'. Don't use for: Vue/Svelte/Angular (use vue-reviewer analog or frontend-engineer), pure HTML/CSS, server-only non-React code, or non-React TS (use typescript-reviewer). Read-only."
