...
The gate is the **fatal-weakness floor**, not a blended score — this matches how this harness already gates decisions (`kbg:score-decision`, `ship-merge`): a single number hides which lens is the actual blocker. Full YAML template (`plan_source`, `findings`, `cleared_decoys`, `top_blockers`, `verdict` with its not-ready-first branching logic, `confidence`, `not_reviewed`, `verdict_movers`, `revisit_if`) and the citation-honesty rule preloaded via `kbg:plan-reviewer-format` (see this file's `skills:` frontmatter).
...
verdict) preloaded via `kbg:plan-reviewer-format`.
