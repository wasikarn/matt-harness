---
name: rust-reviewer
description: "Expert Rust code reviewer — ownership, lifetimes, error handling, unsafe usage, idiomatic patterns. Use for all Rust code changes (MUST BE USED for Rust projects) touching .rs files. Use when reviewing .rs files, ownership/lifetime/unsafe code, or any Rust change request. Thai: 'review Rust', 'Rust review', 'rs', 'ownership', 'lifetime', 'ตรวจ Rust', 'โค้ด Rust'. Don't use for: C/C++ (use cpp-reviewer analog), other systems languages without ownership semantics, or non-systems code. Read-only."
