...
run_hook_tests() {
  local rc=0 t
  for t in "$ROOT/tests/hooks/test-gates.sh" "$ROOT/tests/hooks/test-worktree-guard.sh" "$ROOT/tests/hooks/test-verifier-protect.sh" "$ROOT/tests/hooks/test-flow-nudge.sh" "$ROOT/tests/hooks/test-jira-route-nudge.sh" "$ROOT/tests/hooks/test-session-stop.sh" "$ROOT/tests/hooks/test-learn-nudge.sh" "$ROOT/tests/hooks/test-memory-health-nudge.sh" "$ROOT/tests/hooks/test-instructions-loaded-journal.sh" "$ROOT/tests/hooks/test-skill-usage-telemetry.sh" "$ROOT/tests/hooks/test-precompact-state-flush.sh" "$ROOT/tests/hooks/test-dispatch-pretooluse.sh" "$ROOT/tests/hooks/test-dispatch-single.sh" "$ROOT/tests/hooks/test-credential-guard.sh" "$ROOT/tests/hooks/test-config-write-guard.sh" "$ROOT/tests/skills/harness-audit/test-harness-audit.sh" "$ROOT/tests/skills/inventory/test-inventory-witness.sh" "$ROOT/tests/scripts/test-run-gauntlet-wiring.sh" "$ROOT/tests/scripts/test-mattpocock-root-resolver.sh" "$ROOT/tests/scripts/test-hook-wired-via-pretooluse-table.sh" "$ROOT/tests/git-hooks/test-pre-commit-version-gate.sh" "$ROOT/tests/git-hooks/test-pre-commit-loc-gate.sh" "$ROOT/tests/commands/test-cost-report.sh" "$ROOT/tests/commands/test-ship-merge-codeowners.sh" "$ROOT/tests/commands/test-risk-check.sh"; do
    [ -f "$t" ] || continue
    bash "$t" 2>&1 || rc=1
  done
