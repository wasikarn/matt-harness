...
run_hook_tests() {
  local rc=0 t
  for t in "$ROOT/hooks/tests/test-gates.sh" "$ROOT/hooks/tests/test-worktree-create.sh" "$ROOT/hooks/tests/test-worktree-guard.sh" "$ROOT/hooks/tests/test-flow-nudge.sh" "$ROOT/hooks/tests/test-jira-route-nudge.sh" "$ROOT/hooks/tests/test-session-stop.sh" "$ROOT/hooks/tests/test-learn-nudge.sh" "$ROOT/skills/harness-audit/tests/test-harness-audit.sh"; do
    [ -f "$t" ] || continue
    bash "$t" 2>&1 || rc=1
  done
  return "$rc"
