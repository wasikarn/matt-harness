...
def digest_stream(name, rows, cat_idx):
    ...
    cats = Counter(c for c in (category(line, cat_idx) for _, line in rows) if c)
...
def main():
    ...
    for name, cat_idx in TAB_STREAMS.items():
        ...
        if len(rows) / span > RUNAWAY_PER_DAY:
            ...
            vals = Counter(line.split("\t")[cat_idx] for _, line in rows
                           if cat_idx is not None and len(line.split("\t")) > cat_idx)
