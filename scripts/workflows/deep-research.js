...
//
// Canonicalized 2026-07-30 from 8 ad-hoc inline runs (2026-07-03 through 2026-07-18) that had
// re-authored this same ~23KB script from scratch each time instead of saving it once. Two
// concrete fixes applied on top of the proven baseline (unchanged across the 07-14/07-16/07-17
// runs) rather than the most-recent "-haiku" variant (07-18 x3), which routed ALL five stages
// including Verify and Synthesize to Haiku — model-tiering here is selective: Haiku only for the
// mechanical/classification stages (Scope, Search, Fetch/Extract), session-default model kept for
// the two judgment-heavy stages (Verify's adversarial voting, Synthesize's final report) where a
// wrong answer is expensive. See CHANGELOG for the source comparison.
...
// Judgment stage — deliberately NOT routed to a cheaper model (see file header).
phase("Verify")
...
// counterSource: collected by VERDICT_SCHEMA since the original version but never surfaced —
// cite the first refuting vote that named one, so a killed claim's transparency block shows what
// disputed it, not just that something did.
const toRefuted = c => {
  const counter = c.verdicts.find(v => v.refuted && v.counterSource)
  return {
    claim: c.claim,
    vote: (c.verdicts.length - c.refutedVotes) + "-" + c.refutedVotes,
    source: c.sourceUrl,
    ...(counter ? { counterSource: counter.counterSource } : {}),
  }
}
...
// ─── Synthesize ───
// Highest-stakes single output of the run — deliberately NOT routed to a cheaper model (see file header).
phase("Synthesize")
