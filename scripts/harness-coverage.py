...
def _load_registry(path):
    ...
    try:
        ...
        print(f"error: malformed JSON in {path}: {e}", file=sys.stderr); sys.exit(1)
    if not isinstance(data, dict) or "sensors" not in data:
        print(f"error: {path} missing top-level 'sensors' key", file=sys.stderr); sys.exit(1)
    if not isinstance(data["sensors"], list):
        print(f"error: {path} 'sensors' is not a list", file=sys.stderr); sys.exit(1)
    return data
...
def main(argv=None):
    ...
    if args.window_sessions <= 0:
        print("error: --window-sessions must be > 0", file=sys.stderr); return 1
    if args.weight_meaningful < 0 or args.weight_trivial < 0:
        print("error: weights must be non-negative", file=sys.stderr); return 1
