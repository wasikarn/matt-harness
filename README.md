...
A **personal Claude Code harness** delivered as an installable plugin (`kbg@kobig`). It adds 31 specialist agents, 71 workflow skills, and 42 slash commands — plus two output-style registers and a terminal theme. No symlink farm, no manual wiring: components auto-discover from the plugin cache.
...
| **Skills** | 71 | Invoke via `kbg:<skill>` (e.g. `kbg:thinking`, `kbg:decide`, `kbg:grilling`) |
| **Commands** | 42 | Invoke via `/<command>` (e.g. `/review-pr`, `/plan`, `/feature-dev`) |
