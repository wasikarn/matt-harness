...
[![Version](https://img.shields.io/badge/version-v0.68.476-blue)](CHANGELOG.md)
...
claude plugin details mh@kobig   # component inventory + token cost — folds all flat-file
                                   ...
                                   # directory-form commands (review-fixtures) at all, so
                                   # this won't confirm those specifically; use step 7's
                                   # smoke test for a real functional check instead
```
...
|---|---|---|
| **Skills** | 48 | `mh:<skill>` — e.g. `mh:pr`, `mh:orchestrate` (matt-origin skills install as a separate namespaced plugin — e.g. `mattpocock-skills:grilling`) |
| **Agents** | 16 | Spawned by Claude or via the `Task` tool — e.g. `code-architect` |
| **Commands** | 9 | `/mh:<command>` — e.g. `/mh:post-mortem`, `/mh:ship-release` (namespaced identically to skills — see Quick Start step 7; shrinking as the commands→skills convergence lands, spec #101) |
...
> `ls commands/` (real current fleet: 48 skills · 16 agents · 9 commands).
...
| kbg-native | MIT | 48 skills · 16 agents · 9 commands |
