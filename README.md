...
A **personal Claude Code harness** delivered as an installable plugin (`kbg@kobig`). It adds 49 specialist agents, 106 workflow skills, and 68 slash commands — plus two output-style registers and a terminal theme. No symlink farm, no manual wiring: components auto-discover from the plugin cache.
...
| **Skills** | 106 | Invoke via `kbg:<skill>` (e.g. `kbg:thinking`, `kbg:decide`, `kbg:grilling`) |
...
├── skills/               # 106 workflow skills (one dir each: SKILL.md + optional references/)
