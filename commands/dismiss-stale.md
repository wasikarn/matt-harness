...
disable-model-invocation-reason: governance — the model must not mute its own staleness/safety alert
---
...
- **Operator-only.** `disable-model-invocation: true` is mandatory. The harness never invokes this on the operator's behalf — the autonomy invariant (the no-model-self-start rule in METHODOLOGY.md and CLAUDE.md §The operating model) forbids the model from self-muting its own coverage alerts.
