...
- **Delegate bugs.** Phase 4 routes bug-shaped comments to `/fix-bug` instead of fixing inline — gets the full reproduce → minimise → hypothesize → TDD discipline.
...
branch), bug-shaped (`/fix-bug`) vs. inline-edit routing (grep every caller, fix sibling call sites
...
**Anti-patterns**: see `review-pr/reference.md` §"Anti-patterns (author)" — silent push, performative agreement, defensive tone. The sha citation + one-line summary IS the acknowledgment; skip the social ceremony. Silent push (fixing without replies) is the failure mode this command exists to prevent.
...
See `references/integration-notes.md` — METHODOLOGY alignment, memory dependencies, `/fix-bug` delegation, active hooks, agent-routing, thread auto-resolve default, and fork-PR handling.
