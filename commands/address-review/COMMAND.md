...
- **Zero open actionable threads on exit.** Phase 5 is non-negotiable — every actionable thread gets a reply (sha citation, wontfix rationale, or clarifying question). Pushing fixes silently is incomplete work (encoded in memory: `feedback_reply_after_pr_fix`). One exception: if the user explicitly says mid-session they're abandoning the PR (see Phase 7's "abandoned" bullet), that instruction overrides this rule — don't insist on replying to the remaining threads before honoring it, and don't invent a reason those threads were exempt from triage in the first place.
...
3. Fetch overall reviews: `gh pr view <n> --json reviews` — capture review-level state (CHANGES_REQUESTED / APPROVED / COMMENTED), which reviewer left it, and the review's own **`body`** field. A non-empty `body` (reviewers routinely put the real ask here, not just in line comments) is a first-class item to triage in Phase 2 — it has no thread to reply into or resolve, so address it in code and acknowledge it in the Phase 5 summary rather than expecting a thread-reply.
...
1. **Pre-implement verification** (per cluster, before editing — external reviewers including LLM-based ones often lack codebase context):
   ...
   - **Author-aware dedup**: skip re-firing a fix on a thread already addressed *more recently* than the reviewer's comment by a commit on this branch authored by the PR's own author (compare the thread's `originalCommit.oid` / `created_at` from Phase 1 against `git log --format='%H %an %s' <headRefName>` filtered to that author — this covers the whole branch history, not just commits made earlier in this Phase 4 pass). Re-acting on an already-handled thread is how an uncapped fix loop churns no-op commits. Same "already resolved, just not by this pass" situation as the `isOutdated` case above — handle it the same way: re-classify `wontfix`, cite the pre-existing sha via the "addressed in `<other-place>`" shape, and surface it alongside the other reclassified clusters. Don't leave it silently `actionable` with nothing in the Phase 4 sha mapping — Phase 5 still needs a reply for it.
2. For each verified actionable cluster (in Phase 3 order):
   ...
   - **Inline edit** → **grep every caller of the affected function/symbol before editing** (code-review-graph MCP `query_graph_tool` if the repo's graph is built — check `mcp__code-review-graph__list_repos_tool` first; else Grep for the symbol). If a sibling call site exhibits the same defect, fix all siblings in this same commit — one cluster, not N round-trips. This is the Ponytail root-cause rule: a guard in the shared function is a smaller diff than a guard in every caller, and patching only the reported instance leaves every sibling still broken (the exact point-fix pattern that stretched the tathep `compliance-audit-round-2` loop past 10 rounds — a fix in one caller introduced a new finding in another that the next review-pr pass surfaced). Then commit with a focused message (reference the thread: `fix(auth): null-check user_id (review #thread-1)`). Capture the sha.
