...
4. Note current review approvals: `gh pr view <n> --json reviews` — informational only, not a Phase 1 gate criterion (GitHub doesn't count a PR author's own approval, so Approval status could never clear for a self-authored PR). If branch protection requires an approval, Phase 2 steps 4–5 are the real enforcement point — GitHub blocks the merge without either a genuine approval or the explicit `--admin` bypass confirmed there.
