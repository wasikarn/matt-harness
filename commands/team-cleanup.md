...
1. Run `bash scripts/lock-reap.sh --team=<slug> [--dry-run]`.
...
**Actions (per plan):**
1. Acquire the board lock via `task_board_lib.py` (or `scripts/lock-claim.sh` on the plan directory), or use atomic read/write discipline.
...
**Actions (per plan):**
1. Run `bash scripts/mailbox-reap.sh --team=<slug>`.
...
- **Lock reaper** — `scripts/lock-reap.sh` breaks expired locks and logs via `journal_append`.
- **Mailbox reaper** — `scripts/mailbox-reap.sh` removes old unread and archived messages.
