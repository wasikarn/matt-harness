...
Import `scripts/task_board_lib.py` (relative to the repo root) and call `board_read(plan_dir)` where `plan_dir` is the directory containing `board.json`.
...
# Add repo root to path so task_board_lib can be imported
...
4. Run the script with `python3 .scratch/wave-status.py <plan_dir>`.
