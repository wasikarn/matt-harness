...
| _codeowners_match.py | Shared CODEOWNERS discovery + matching logic, used by both commands/ship-merge.md's step 7 (CLI wrapper below, argv contract is 4 positional args as of the #50 head-SHA-pinning fix, 2026-08-15 — codeowners content, changed files, reviews JSON, plus a required head_sha) and hooks/gates/convergence-merge-gate.sh (imports evaluate()/discover() in-process, no subprocess spawn). |
...
---
_Generated: 2026-08-15T11:38:03Z_
...
- **Tests:** harness-audit (59 checks) + a 14-file hook behavioral suite, run in parallel by `scripts/run-gauntlet.sh` — see `CLAUDE.md`'s Validation section. The old critical-hooks suite + eval dataset gate were deleted, not rebuilt, in the 2026-06-27 reset (`c452102`). (Check/test counts here are hand-maintained — keep in sync with `ls skills/harness-audit/scripts/checks/*.sh | wc -l` and the test list in `scripts/run-gauntlet.sh`.)
