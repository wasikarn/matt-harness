...
### Commands (21)
  ...
  ◇ ideate-search                  Search past kbg:ideate runs. Type `/ideate-search <query>` to find problems and outputs from previous ideate sessions using the local qmd collection (e.g., `/ideate-search caching`).
  ◇ ideate                         Explicit user entry point for divergent ideation. Type `/ideate <problem>` to run the kbg:ideate skill with a clear opt-in signal, bypassing the auto-fire pre-flight gate. Advisory-only: it invokes the existing ideate skill rather than duplicating the algorithm.
...
---
_Generated: 2026-06-17T08:46:26Z_
