...
| Hook | Purpose |
|---|---|
| agent-write-scope.sh | Sensor (not a gate) -- per-agent write-scope journal, issue #137, rev 4 (log-mode). NEVER denies anything -- Write/Edit/MultiEdit/NotebookEdit and Bash `git commit` always proceed unmodified. Two legs, one script (same shape as main-exec-guard.sh and subagent-git-guard.sh), wired under two separate matchers -- a recorder on every subagent write and an enforcer that journals unrecorded paths on a subagent's `git commit`. |
...
| flow-nudge.sh | Advisory: when the user's prompt looks like non-trivial engineering work, nudge plan-first — enter plan mode (Shift+Tab / EnterPlanMode) before editing, with the heavyweight spec flow (the user types /mattpocock-skills:grill-with-docs first, then /mattpocock-skills:to-spec → /mattpocock-skills:to-tickets → /mattpocock-skills:implement; grill-with-docs, to-spec, to-tickets, and implement are all disable-model-invocation upstream — never point the model at grill-with-docs as something IT can call, only bare grilling on the multi-file/one-way-door line below is model-invocable) as the branch for a feature to spec out. UserPromptSubmit hook. Output → plain stdout — docs: "added as context Claude can see and act on" (not the JSON hookSpecificOutput.additionalContext path, which is […] |
...
| irrecoverable.py | A malformed/absent payload must fail closed, not silently become "no command to check" — that collapse let empty stdin, truncated JSON, and tool_input:null bypass every check below (found 2026-08-06). |
...
| _hook_output.py | Shared hook-output JSON primitive. Used by hooks/gates/db-write-gate.sh's embedded python3 -c block and hooks/gates/verifier-protect.py (imported via sys.path.insert -- GH #146 extracted verifier-protect's own python out of an embedded python3 -c block into that sibling file), both of which defined an identical emit_ask() before this extraction (2026-08-15) -- each gate still builds its own reason message inline (that part is legitimately gate-specific), only the JSON-shape emission is shared here. |
...
| merge-door.py | Insert a literal ";" after each real newline (not in place of it, so a following "#" comment still stops there) -- a backslash immediately before the newline is a real line continuation, not a separator: bash removes BOTH characters entirely, joining the two lines with nothing between them, so nothing is emitted for that pair either (2026-09-03, GH #126 -- mirrors the same fix already shipped in irrecoverable.sh / main-exec-guard.sh / worktree-guard.py / verifier-protect.sh). The prior version put the literal "\<newline>" back unchanged, which left a stray embedded newline glued onto a token when there was no whitespace around the continuation (e.g. "g\<newline>h pr merge 123" tokenized argv0 as "g\nh", never "gh") -- confirmed live, ground-truthed against real bash, and a genuine bypass of the exact-match argv0/token […] |
...
| subagent-git-guard.sh | Gate: a subagent may not run a repo-wide-mutating git subcommand via Bash -- only the main session may, and only because nothing else is writing in the shared tree. Reads the PreToolUse JSON payload from stdin; exits 2 to block. |
...
| verifier-protect.py | lib_dir is argv[1] -- $(dirname "$0")/lib, passed by the bash wrapper's invocation of this script. $0-based resolution because CLAUDE_PLUGIN_ROOT can be empty in some invocation contexts; $0 cannot, since bash sets it from the literal command that ran this file. |
...
| test-agent-write-scope.sh | Behavioral tests for hooks/advisory/agent-write-scope.sh (issue #137, rev 4 log-mode sensor). This gate NEVER exits non-zero on either leg, so every assertion here reads the JOURNAL FILE CONTENT (recorder: the per-agent ~/.local/share/kbg/mh-agent-writes/<id>.jsonl; enforcer: the shared ~/.local/share/kbg/metrics/agent-write-scope.jsonl), not the exit code. Every run sets HOME to a throwaway fixture so those paths never touch the operator's real ~/.local. Enforcer cases run inside throwaway git fixtures (mktemp -d, git init) so they never touch the real matt-harness working tree. Run standalone: bash tests/hooks/test-agent-write-scope.sh |
...
| test-subagent-git-guard.sh | Behavioral tests for hooks/gates/subagent-git-guard.sh (issue #135). Run standalone: bash tests/hooks/test-subagent-git-guard.sh |
...
---
_Generated: 2026-09-04T17:58:40Z_
...
Canonical file patterns per agent. Assign each file to exactly one agent in an `orchestrate` dispatch plan to prevent silent overwrites. This table is a hand-maintained literal, not generated from `agents/*.md` — keep it in sync by hand when an agent is added or removed. Not covered by harness-audit check 12 (that check only verifies `skills/workflow/orchestrate/SKILL.md` + `reference.md` + `routing.md`, not this table or `BOUNDARY.md`) — `inventory-boundary.sh` prints a stderr warning at regen time if this table and `agents/` disagree, but that's advisory, not a CI gate.
